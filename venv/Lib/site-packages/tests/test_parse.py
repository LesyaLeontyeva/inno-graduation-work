...
class GoodNewestResponse(object):
    status_code = 200
    xml = os.path.join(os.path.dirname(__file__), "newest.xml")
    content = open(xml).read()


class GoodUpdatedResponse(object):
    status_code = 200
    xml = os.path.join(os.path.dirname(__file__), "updated.xml")
    content = open(xml).read()
...
class TestParse(unittest.TestCase):
    ...
    def test_newest_package(self, get_mock):
        ...
        self.assertEqual("Automatic functional testing for CLI apps.", p[0].description)
        self.assertEqual(
            datetime.strptime("09 Aug 2014 06:57:42 GMT", "%d %b %Y %H:%M:%S %Z"),
            p[0].date,
        )
    ...
    def test_updated_packages(self, get_mock):
        ...
        self.assertEqual(
            call("https://pypi.python.org/pypi?%3Aaction=rss"), get_mock.call_args
        )
    ...
    def test_updated_package(self, get_mock):
        ...
        self.assertEqual(
            datetime.strptime("09 Aug 2014 08:40:20 GMT", "%d %b %Y %H:%M:%S %Z"),
            p[0].date,
        )
